...
// Strip parenthetical qualifier from outcome name: "Valeri Nichushkin (First Goalscorer)" → "Valeri Nichushkin"
// Used for outcomes that include market context in name (es. hockey Player Props con
// First Goalscorer suffix). Trailing-only paren stripped; preserved se in mezzo al nome.
function stripQualifier(name: string): string {
  const m = name.match(/^(.+?)\s*\([^()]+\)\s*$/);
  return m ? m[1].trim() : name;
}

// Strip "::over"/"::under" outcome separator (defensive — should be routed away from
// PlayerListFlat, ma fallback se entry slip past detection).
function stripOverUnderSep(name: string): string {
  return name.replace(/::(over|under)$/i, "").trim();
}

function cleanPlayerName(raw: string): string {
  return stripQualifier(stripOverUnderSep(raw));
}
            ...
            <button
              ...
              onClick={() => {
                ...
                onSelect({
                  ...
                  odds: p.odds,
                  label: cleanPlayerName(p.playerName),
                });
              }}
              style={{
                fontSize: 15,
