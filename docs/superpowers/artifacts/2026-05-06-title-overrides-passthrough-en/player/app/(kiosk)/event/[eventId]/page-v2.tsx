...
export default function EventDetailPageV2({ event, eventId, onSelectOutcome }: Props) {
  ...
  function renderSingleMarket(adapted: { _ref: DbMarket }, isPrincipali: boolean) {
    ...
    // OverUnder-flat detection (PRIMA di PLAYER_FLAT_TYPES): se gli outcome usano formato
    // "PlayerName::over"/"PlayerName::under" sono Over/Under per linea per giocatore →
    // PlayerOverUnderRow è il rendering corretto, non PlayerListFlat (che assume 1 odds/player).
    const hasOverUnderOutcomes = (m.outcomes ?? []).some((o) => /::(over|under)$/.test(o?.name ?? ""));
    if (hasOverUnderOutcomes) {
    ...
    const PLAYER_FLAT_TYPES = new Set([
      ...
      "Player Passes",
      // Player Props catch-all (es. hockey First Goalscorer wrapped) — outcome name
      // pattern "PlayerName (Qualifier)" parsed by PlayerListFlat strip helper
      "Player Props",
