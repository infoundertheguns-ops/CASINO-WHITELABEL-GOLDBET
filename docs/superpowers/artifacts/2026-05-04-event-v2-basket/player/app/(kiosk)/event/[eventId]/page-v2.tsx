...
// Resolve a basket sport-specific title override. Returns the uppercased label or
// null when no override exists. Hoisted so BOTH titleFor and renderGroupedMarket can
// use it consistently — @picker markets render their MarketSection title from
// renderGroupedMarket and would otherwise bypass the override map.
...
export default function EventDetailPageV2({ event, eventId, onSelectOutcome }: Props) {
  ...
  function renderGroupedMarket(spec: string, variants: DbMarket[]) {
    ...
    if (
      ...
      return (
        <MarketSection key={spec} title={(sportSlug === "basket" ? resolveBasketOverride(marketType) : null) ?? marketType.toUpperCase()}>
          <AsianHandicapBlock
