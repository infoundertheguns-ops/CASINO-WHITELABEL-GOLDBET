...
const _TENNIS_NOISE = new Set([
  ..._DEFAULT_NOISE,
  // Single-letter player initials that collide with team-category reserve markers.
  // In tennis (individual sport) there is no "B team" or "C team" — "b"/"c" after
  // stripping a period from "Alcaraz C." would otherwise land in reserveMarkers via
  // _DEFAULT_RESERVE, causing a false Stage-1 mismatch. Silencing them here (NOISE
  // filter runs before reserve detection) keeps them out of the token stream entirely.
  "b", "c",
