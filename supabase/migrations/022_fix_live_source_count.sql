-- ═══ 022: Fix source_markets_count for live events ═══
-- Problem: upsert_live_batch never updated source_markets_count.
-- The prematch value (set with GREATEST) stayed even after events went live,
-- inflating the expected count because Kambi offers far fewer markets live.
-- Fix: set source_markets_count = actual live market count (no GREATEST).
...
BEGIN
  ...
  LOOP
    BEGIN  -- per-event exception block
      ...
      -- Live events have fewer markets than prematch (Kambi suspends many).
      -- Do NOT use GREATEST here — the count must reflect current reality.
