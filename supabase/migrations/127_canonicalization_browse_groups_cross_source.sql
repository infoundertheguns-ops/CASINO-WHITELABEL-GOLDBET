-- Migration 127: rewrite canonicalization_browse_groups + inspect_event to
-- recognize events.canonical_id (mig 126) as a grouping key with priority
-- BETWEEN flashscore_id (highest) and trigram-on-the-fly (fallback).
--
-- Sprint 3 Phase A — surfaces the synthetic cross-source clusters minted by
-- assign_event_cross_source_canonical_id(uuid). After backfill, those clusters
-- now appear in the Esplora page as group_type='cross_source' with all 3
-- source cards instead of being flagged "❌ Single source only".
--
-- Baseline: mig 125 (grouped-counts edition with mig 124 fast-path preserved).
-- The ONLY behaviour change is:
--   1. enriched CTE now selects e.canonical_id.
--   2. grouping CASE adds a new branch BETWEEN flashscore and trigram:
--        WHEN flashscore_id IS NOT NULL → 'fs:' || flashscore_id
--        WHEN canonical_id  IS NOT NULL → 'cs:' || canonical_id   ← NEW
--        WHEN trigram pair exists       → 'trigram:' || cluster_id
--        ELSE 'iso:' || id
--   3. group_type label maps 'cs:%' → 'cross_source'.
--   4. event JSON `canonical_id` field carries the actual UUID (was NULL).
--   5. field_signals.canonical_id: 'ok_synthetic' when populated,
--      'absent_problem' when null AND no flashscore_id (was 'feature_pending').
--
-- All other CTE bodies, output JSON shape, security flags, statement_timeout,
-- search_path, partial-index assumptions, and trigram fast-path logic from
-- mig 124+125 are preserved verbatim.
--
-- Rollback: re-apply mig 125. The two added partial indexes from mig 125 are
-- harmless to keep.
...
-- ═══════════════════════════════════════════════════════════════════
-- canonicalization_browse_groups — cross-source aware
-- ═══════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════
-- inspect_event — same cross-source grouping rule
-- ═══════════════════════════════════════════════════════════════════
