...
BEGIN
    RETURN (
        WITH event_counts AS (
            SELECT
                e.id AS event_id, e.external_id, e.home_team, e.away_team,
                e.starts_at, e.source, e.source_markets_count,
            ...
            GROUP BY e.id, s.name, s.slug, l.name, e.status
        ),
        summary AS (
            ...
            FROM (
                SELECT jsonb_build_object(
                    ...
                    'gap_pct', CASE
                        WHEN COUNT(*) FILTER (WHERE source_markets_count IS NOT NULL) > 0
                            AND AVG(source_markets_count) FILTER (WHERE source_markets_count IS NOT NULL) > 0
                        ...
                        ELSE NULL
                    END,
                    'gap_total', COALESCE(SUM(GREATEST(source_markets_count - vincitu_count, 0))
                                FILTER (WHERE source_markets_count IS NOT NULL), 0),
        ...
        gap_events AS (
            ...
            FROM (
                SELECT jsonb_build_object(
                    'event_id', event_id, 'external_id', external_id,
                    'home_team', home_team, 'away_team', away_team,
                    'sport_name', sport_name, 'sport_slug', sport_slug,
                    'league_name', league_name, 'source', source,
                    'status', status_bucket,
        ...
        availability AS (
            SELECT
                COUNT(*) AS total,
                COUNT(*) FILTER (WHERE source_markets_count IS NOT NULL) AS with_source
            FROM event_counts
        )
        SELECT jsonb_build_object(
