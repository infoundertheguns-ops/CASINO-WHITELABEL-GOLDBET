-- Migration 133: canonicalization_overview() L3 — populate cross_source_canonical.
--
-- Sprint 3 Phase B follow-up to mig 130. Mig 130 left
-- 'cross_source_canonical' and 'cross_source_pct' hardcoded to 0 with the
-- comment "Task #2 pending"; the frontend overview-tab.tsx:83 surfaces this
-- literally as "🚧 Cross-source canonical: 0 (Task #2 pending)". Phase A.2
-- has since backfilled ~328 cross-source clusters → ~736 active events with
-- events.canonical_id NOT NULL on prod, so the placeholder is now misleading.
--
-- This migration:
--   - Counts active 7d events with canonical_id IS NOT NULL → v_cross_source_canonical
--   - Counts DISTINCT canonical_id values on the same window → v_cross_source_clusters
--   - Computes cross_source_pct = v_cross_source_canonical / v_total_events_active * 100
--
-- All other fields (source_only_flagged, mappable_total, coverage_among_mappable_pct,
-- per_stage, per_source, color logic) are preserved verbatim from mig 130.
--
-- Idempotent: CREATE OR REPLACE.
-- Rollback: re-apply mig 130.
...
DECLARE
  ...
  v_cross_source_canonical int;
  v_cross_source_clusters int;
BEGIN
  -- Level 1: sports
  ...
  -- Sprint 3 Phase A.2 (mig 128): cross-source canonical_id coverage on same 7d window.
  -- Counts both events linked to a cluster and distinct cluster ids — count of clusters
  -- is the upper-bound recovery (each cluster is one "real-world" event de-duplicated).
  SELECT
    count(*) FILTER (WHERE canonical_id IS NOT NULL),
    count(DISTINCT canonical_id) FILTER (WHERE canonical_id IS NOT NULL)
  INTO v_cross_source_canonical, v_cross_source_clusters
  FROM events
  WHERE status IN ('prematch', 'live')
    AND home_team NOT IN ('Home', 'Home (Special bets)')
    AND home_team NOT LIKE '% +'
  ...
  v_result := jsonb_build_object(
    ...
    'level_3_events', jsonb_build_object(
      ...
      'cross_source_canonical', v_cross_source_canonical,
      'cross_source_clusters', v_cross_source_clusters,
      'cross_source_pct', CASE WHEN v_total_events_active > 0
        THEN round((v_cross_source_canonical::numeric / v_total_events_active) * 100, 1)
        ELSE 0 END,
