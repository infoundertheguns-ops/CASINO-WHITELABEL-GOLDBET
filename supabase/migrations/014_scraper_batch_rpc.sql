-- ============================================================
-- MIGRATION 014: Batch RPC for prematch + live scraper
-- Replaces ~80 sequential HTTP round-trips per batch with 1 RPC call
-- ============================================================

-- Required for slugify() — remove accents
CREATE EXTENSION IF NOT EXISTS unaccent;

-- ═══════════════════════════════════════════════════════════
-- HELPER: slugify(TEXT) → TEXT
-- Mirrors the JS slugify: lowercase, remove accents, replace non-alnum with dashes, trim dashes
-- ═══════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════
-- HELPER: extract_line(TEXT) → NUMERIC
-- Extracts the betting line from market type name (e.g. "OVER_UNDER_2.5" → 2.5)
-- Returns NULL if no line found or if it's a player prop ID (>= 1000000)
-- ═══════════════════════════════════════════════════════════
CREATE OR REPLACE FUNCTION extract_line(market_type TEXT)
RETURNS NUMERIC
LANGUAGE sql IMMUTABLE STRICT
AS $$
  SELECT CASE
    WHEN m[1] IS NOT NULL AND abs(m[1]::NUMERIC) < 1000000 THEN m[1]::NUMERIC
    ELSE NULL
  END
  FROM (
    SELECT regexp_match(market_type, '[_ ](-?\d+\.?\d*)$') AS m
  ) sub;
$$;

-- ═══════════════════════════════════════════════════════════
-- HELPER: sport_icon(TEXT) → TEXT
-- Returns emoji icon for a sport slug
-- ═══════════════════════════════════════════════════════════
CREATE OR REPLACE FUNCTION sport_icon(sport_slug TEXT)
RETURNS TEXT
LANGUAGE sql IMMUTABLE STRICT
AS $$
  SELECT COALESCE(
    (SELECT v FROM (VALUES
      ...
      ('pallavolo','🏐'),('tennis-tavolo','🏓'),('rugby','🏉'),('baseball','⚾'),
      ('mma','🥊'),('ciclismo','🚴'),('esports','🎮'),('handball','🤾'),
      ('freccette','🎯'),('football','🏈'),('boxe','🥊'),('snooker','🎱'),
      ('cricket','🏏'),('sport-invernali','⛷️'),('motori','🏎️')
    ) AS icons(k, v) WHERE k = sport_slug),
    '⚽'
  );
$$;
...
-- ═══════════════════════════════════════════════════════════
-- FUNCTION 1: upsert_prematch_batch(JSONB)
--
-- Input: { "events": [ { external_id, sport, league, home_team, away_team, starts_at, status?, markets: [{ type, outcomes: [{ name, odds }] }], overview_only? } ] }
-- Output: { "processed": N, "errors": [...] }
--
-- Logic per event:
--   1. Upsert sport by slug → sport_id
--   2. Upsert league by slug → league_id
--   3. Find event by external_id; if is_live=true, SKIP
--   4. Update or insert event
--   5. Upsert markets (ON CONFLICT event_id, market_type)
--   6. If !overview_only: deactivate stale markets + outcomes
--   7. Upsert outcomes (ON CONFLICT market_id, name), filter odds > 1
-- ═══════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════
-- FUNCTION 2: upsert_live_batch(JSONB)
--
-- Input: { "events": [ { external_id, status?, minute?, home_score?, away_score?, period?, period_code?, half_score_home?, half_score_away?, stats?, match_events?, home_team?, away_team?, sport?, league?, starts_at?, markets?: [{ type, outcomes: [{ name, odds }] }] } ] }
-- Output: { "updated": N, "inserted": N, "errors": [...] }
--
-- Logic per event:
--   1. Find by external_id (fallback: home_team+away_team where is_live)
--   2. If not found + has creation fields → auto-create (sport + league + event)
--   3. Update: status, score, minute, period, live_data
--   4. Detect ended: period in ENDED/FINISHED/FULL_TIME/etc → status=finished, is_live=false
--   5. If markets present:
--      a. Suspend all outcomes of active markets for this event
--      b. Upsert markets
--      c. Upsert outcomes (un-suspends those in payload)
--      d. Deactivate stale markets + their outcomes
-- ═══════════════════════════════════════════════════════════
  ...
  LOOP
    BEGIN  -- per-event exception block
      ...
      IF v_event_id IS NULL AND v_ev ->> 'home_team' IS NOT NULL AND v_ev ->> 'away_team' IS NOT NULL THEN
        ...
        IF v_event_id IS NOT NULL THEN
          -- Patch external_id on the matched event
      ...
      IF v_event_id IS NULL THEN
        ...
        INSERT INTO leagues (sport_id, name, slug, is_active)
        VALUES (v_sport_id, v_league_name, v_league_slug, TRUE)
        ON CONFLICT (slug) DO UPDATE SET name = EXCLUDED.name, sport_id = EXCLUDED.sport_id
        RETURNING id INTO v_league_id;
