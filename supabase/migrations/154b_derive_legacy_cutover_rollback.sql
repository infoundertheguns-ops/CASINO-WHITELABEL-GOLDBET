-- supabase/migrations/154b_derive_legacy_cutover_rollback.sql
-- Restore pre-cutover derive_legacy_from_v2() body verbatim (captured 2026-04-29
-- from prod via pg_get_functiondef). Run this to undo 154b cutover.
--
-- Note: this rollback does NOT reactivate markets that the post-cutover cleanup
-- deactivated. Reactivation happens automatically on the next derive cron run
-- via ON CONFLICT DO UPDATE setting is_active=true on the (event, market_type)
-- pairs that exist in the broader (pre-filter) source set.
