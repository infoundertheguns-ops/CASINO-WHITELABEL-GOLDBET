-- supabase/migrations/156_derive_legacy_filter_v2_rollback.sql
-- Restores derive_legacy_from_v2 to pre-156 state:
--   - statement_timeout 600s -> 180s
--   - removes Part 3b deactivate pass
--   - removes Part 4 outcomes is_active filter
--
-- After running, manually re-activate any markets that 156 deactivated:
--   UPDATE markets m SET is_active=true, updated_at=now()
--   FROM events e WHERE m.event_id=e.id AND e.source='odds-api'
--     AND e.starts_at > now()-interval '2 days' AND e.starts_at < now()+interval '14 days'
...
DELETE FROM _migrations WHERE name = '156_derive_legacy_filter_v2.sql';
