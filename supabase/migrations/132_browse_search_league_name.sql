-- Migration 132: extend search filter in browse/inspect RPCs to include league name.
--
-- Bug observed in prod after Sprint 3 Phase B deploy: searching "setka" in the
-- Esplora tab returned 0 results despite 4 active "Setka Cup.*" leagues with
-- events flagged is_source_only=true. Root cause: the ILIKE filter only
-- covered home_team / away_team / external_id / flashscore_id, never the
-- league name. So users naturally searching by league/competition name (the
-- whole point of Phase B's "find structurally source-only stuff") got noise.
--
-- Fix: add `OR l.name ILIKE '%' || p_search || '%'` to the WHERE filter in:
--   - canonicalization_browse_groups (mig 131)
--   - inspect_event (mig 131)
--
-- Everything else (CTEs, trigram clustering, cross-source 'cs:' branch,
-- field_signals semantics including is_source_only=true → 'variant') is
-- preserved verbatim from mig 131.
--
-- Idempotent: CREATE OR REPLACE.
-- Rollback: re-apply mig 131.

SET statement_timeout = '5min';
SET lock_timeout = '2min';

-- ═══════════════════════════════════════════════════════════════════
-- canonicalization_browse_groups — search now includes league name
-- ═══════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════
-- inspect_event — search now includes league name
-- ═══════════════════════════════════════════════════════════════════
