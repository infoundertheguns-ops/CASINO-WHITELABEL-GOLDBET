-- ═══ 021: Multi-source market coverage RPC ═══
-- Generalizes get_leon_coverage() with a source parameter

CREATE OR REPLACE FUNCTION get_source_coverage(p_source TEXT)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  ...
  gap_events JSONB;
BEGIN
  -- KPI globali (per source, escludi giocatori-%)
