...
interface BetfairHeroProps {
  server: ServerData | null;
}

const BETFAIR_ACCENT = "#fbbf24"; // amber — distinct from kambi purple + 22bet orange
...
export function BetfairHeroSection({ server }: BetfairHeroProps) {
  const isOffline = !server || !server.latest;
  // Snapshot shape reuses the "kambi" field name as the scraper payload
  // (legacy schema) — for the betfair server key this holds Betfair's stats.
    ...
    <>
      ...
      <div
        style={{
          ...
          borderLeft: `4px solid ${isOffline ? "#6b7280" : BETFAIR_ACCENT}`,
          borderRadius: 12,
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{
              padding: "2px 8px",
              borderRadius: 4,
              background: `${BETFAIR_ACCENT}22`,
              color: BETFAIR_ACCENT,
              fontSize: 10,
              fontWeight: 700,
              letterSpacing: 0.5,
            ...
            <div style={{ padding: "20px 24px" }}>
              ...
              <div style={{ marginTop: 12 }}>
                {stats.slow_prematch && (
                  <PrematchCyclePanel
                    title="Slow"
                    subtitle="Tutti gli sport prematch, sweep completo"
                    cycle={stats.slow_prematch}
                    accent={BETFAIR_ACCENT}
                  />
                )}
                {stats.fast_prematch && (
                  <PrematchCyclePanel
                    title="Fast"
                    subtitle="Solo eventi entro 3h, marketStartTime filter"
