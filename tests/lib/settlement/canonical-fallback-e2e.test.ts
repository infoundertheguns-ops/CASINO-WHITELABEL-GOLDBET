...
describe("Phase B canonical fallback — end-to-end", () => {
  ...
  describe("odds-api 1X2 - 1T → 1X2_HT (HT 1-1 draw)", () => {
    ...
    it("outcome 'X' → won (HT draw 1-1)", () => {
      expect(settle(ev, "1X2 - 1T", "X", undefined, "calcio", { source: "odds-api", lookups })).toBe(
        "won",
      );
    });

    it("outcome '1' → lost (HT is draw, not home win)", () => {
      expect(settle(ev, "1X2 - 1T", "1", undefined, "calcio", { source: "odds-api", lookups })).toBe(
        "lost",
      );
    });
  });

  describe("odds-api U/O 2.5 - 1T → O/U_HT (HT total = 2)", () => {
    const lookups = buildLookups({
      source: "odds-api",
      markets: [
    ...
    it("Under → won (HT total 2 < 2.5)", () => {
      expect(
        settle(ev, "U/O 2.5 - 1T", "Under", undefined, "calcio", { source: "odds-api", lookups }),
      ).toBe("won");
    });

    it("Over → lost (HT total 2 < 2.5)", () => {
      expect(
        settle(ev, "U/O 2.5 - 1T", "Over", undefined, "calcio", { source: "odds-api", lookups }),
      ).toBe("lost");
    });
  });

  describe("odds-api GG/NG - 1T → GG/NG_HT (HT both teams scored)", () => {
    const lookups = buildLookups({
      source: "odds-api",
    ...
    it("outcome 'Si' → won (HT 1-1, both scored)", () => {
      expect(
        settle(ev, "GG/NG - 1T", "Si", undefined, "calcio", { source: "odds-api", lookups }),
      ).toBe("won");
    });

    it("outcome 'No' → lost", () => {
      expect(
        settle(ev, "GG/NG - 1T", "No", undefined, "calcio", { source: "odds-api", lookups }),
      ).toBe("lost");
    });
  });

  describe("odds-api DC - 1T → DC_HT (HT draw 1-1)", () => {
    const lookups = buildLookups({
      source: "odds-api",
    ...
    it("outcome '1X' → won (HT home 1 >= away 1)", () => {
      expect(settle(ev, "DC - 1T", "1X", undefined, "calcio", { source: "odds-api", lookups })).toBe(
        "won",
      );
    });

    it("outcome 'X2' → won (HT draw qualifies X2)", () => {
      expect(settle(ev, "DC - 1T", "X2", undefined, "calcio", { source: "odds-api", lookups })).toBe(
        "won",
      );
    });

    it("outcome '12' → lost (HT is draw, so 12 loses)", () => {
      expect(settle(ev, "DC - 1T", "12", undefined, "calcio", { source: "odds-api", lookups })).toBe(
        "lost",
      );
    });
  });
  ...
  describe("regex fast-path still has priority over canonical", () => {
    // flashscore native "1X2" has a regex match at MARKET_PATTERNS; canonical
    // lookups should not interfere even when provided.
    const lookups = buildLookups({
      source: "flashscore",
      markets: [{ mt: "1X2", canonical_key: "1x2_ft" }],
    });
    it("flashscore native '1X2' outcome '1' → won (FT home 2 > away 1) via regex", () => {
      expect(settle(ev, "1X2", "1", undefined, "calcio", { source: "flashscore", lookups })).toBe(
        "won",
      );
    });
  });
  ...
  describe("no canonical mapping + no regex → null (settlement layer will void)", () => {
    ...
    it("unknown market → null", () => {
      expect(
        settle(ev, "Some Totally Unknown Market - odds-api edition", "X", undefined, "calcio", {
          source: "odds-api",
          lookups,
        }),
      ).toBeNull();
  ...
  describe("canonical_line precedence over marketLine param", () => {
    // Pick a legacy-source-native market name that doesn't collide with any regex
  ...
  describe("Phase C — period 3h/4h via setIdx derivation", () => {
    ...
    it("odds-api '1X2 - 3Q' canonical 1x2_3h → Q3 home 24 > away 18 won", () => {
      const lookups = buildLookups({
        source: "odds-api",
    ...
    it("odds-api 'U/O 45 - 4Q' canonical u_o_4h → Q4 total 42 < 45 Under won", () => {
      const lookups = buildLookups({
        source: "odds-api",
        markets: [
  ...
  describe("outcome_normalization overrides dictionary (source-specific wins)", () => {
    // odds-api uses "GG/NG - BT" (made-up marker) to force canonical fallback;
